 ...
 */
  ...
  /**
    ...
    */
  public createTransparentOverlayMesh(mergedMesh: BABYLON.Mesh, expressID: number, transparency: number = 0.5): void {
    ...
    // 记录数据，等待批量处理
    this.pendingTransparentOverlayData.push({
      mergedMesh,
      expressID,
      transparency
    });
  ...
  /**
   * 批量创建半透明网格：克隆高亮网格并修改透明度
   */
  public createBatchTransparentOverlayMeshes(): void {
    if (!this.scene || this.pendingTransparentOverlayData.length === 0) return;

    console.log(`开始批量创建 ${this.pendingTransparentOverlayData.length} 个半透明网格`);

    // 第一步：查找所有高亮网格
    const highlightMeshes = this.scene.meshes.filter(mesh =>
      mesh.name.includes('highlight') ||
      mesh.material?.name?.includes('highlight')
    );

    if (highlightMeshes.length === 0) {
      console.log('没有找到高亮网格，无法创建半透明效果');
      return;
    }

    console.log(`找到 ${highlightMeshes.length} 个高亮网格，开始克隆并创建半透明版本`);

    // 第二步：克隆高亮网格并创建半透明版本
    highlightMeshes.forEach(mesh => {
      if (mesh.material) {
        // 克隆网格
        const clonedMesh = mesh.clone(`transparent`, null, true) as BABYLON.Mesh;

        if (!clonedMesh) {
          console.warn(`无法克隆网格 ${mesh.name}`);
          return;
        }

        // 克隆材质
        let transparentMaterial: BABYLON.Material;

        if (mesh.material instanceof BABYLON.StandardMaterial) {
          transparentMaterial = mesh.material.clone(`transparent_${mesh.name}`) as BABYLON.StandardMaterial;
        } else if (mesh.material instanceof BABYLON.PBRMaterial) {
          transparentMaterial = mesh.material.clone(`transparent_${mesh.name}`) as BABYLON.PBRMaterial;
        } else {
          // 对于其他材质类型，创建默认半透明材质
          transparentMaterial = new BABYLON.StandardMaterial(`transparent_default_${mesh.name}`, this.scene);
          transparentMaterial.diffuseColor = new BABYLON.Color3(0.0, 1, 1);
        }

        // 设置透明度
        if (transparentMaterial instanceof BABYLON.StandardMaterial) {
          transparentMaterial.alpha = 0.5;
        } else if (transparentMaterial instanceof BABYLON.PBRMaterial) {
          transparentMaterial.alpha = 0.5;
        } else if (transparentMaterial && 'alpha' in transparentMaterial) {
          (transparentMaterial as any).alpha = 0.5;
        }

        // 应用半透明材质
        clonedMesh.material = transparentMaterial;

        // 设置渲染优化属性
        transparentMaterial.backFaceCulling = false;
        transparentMaterial.separateCullingPass = true;
        transparentMaterial.zOffset = 0.1;

        // 保存到映射表中
        const meshId = mesh.id || mesh.name;
        this.transparentOverlayMeshes.set(Number(meshId), clonedMesh);
        this.simpleTarget?.renderList?.push(clonedMesh);
        this.simpleTarget?.setMaterialForRendering(clonedMesh, transparentMaterial);

        console.log(`为网格 ${mesh.name} 创建了半透明克隆版本`);
      }
    });

    // 清空待处理数据
    this.pendingTransparentOverlayData = [];
  }
   ...
   * 给半透明网格添加高亮边框效果
   */
  public applyHighlightToTransparentMeshes(): void {
    if (!this.scene) return;

    // 获取所有透明覆盖网格
    const transparentMeshes: BABYLON.AbstractMesh[] = []
    this.scene.meshes.forEach(mesh => {
      if (mesh.name.includes('transparent')) {
        transparentMeshes.push(mesh);
      }
    });

    if (transparentMeshes.length === 0) {
      console.log('没有找到透明覆盖网格');
      return;
    }

    console.log(`找到 ${transparentMeshes.length} 个透明覆盖网格，准备添加高亮边框效果`);
    this.applyHighlight(transparentMeshes)
  }
   ...
   * 清除所有半透明网格：销毁克隆的网格
   */
  public clearTransparentOverlayMeshes(): void {
    if (!this.scene) return;

    let disposedCount = 0;

    // 销毁所有存储的半透明网格
    this.transparentOverlayMeshes.forEach((mesh) => {
      if (!mesh.isDisposed()) {
        mesh.dispose();
        disposedCount++;
      }
    ...
    // 清除场景中可能遗留的透明网格
    const transparentMeshes = this.scene.meshes.filter(mesh =>
      mesh.name.includes('transparent_') && mesh.name.includes('highlight')
    );
    ...
    console.log(`清除了 ${disposedCount} 个半透明网格`);
  }
}