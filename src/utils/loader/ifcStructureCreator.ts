...
import { IfcTypes } from '../common/ifcTypes'
...
export const getSpatialTree = (props: Props): null | DataReturn => {
  ...
  Object.keys(relations).forEach((id: string) => {
    ...
    const projectTypeInfo = IfcTypes[property.type] ?? { en: '', cn: '' }
    //  ifc project  顶级节点
    ...
    relations[id].forEach((expressId: string) => {
      ...
      const item = {
        ...
        guid: guid,
        objectType: property.ObjectType,
        type: childTypeInfo.en,
        typeShow: childTypeInfo.cn || childTypeInfo.en,
      }
      ...
      if (!dataMap[expressId]) {
        ...
        if (isc) {
          ...
          const categoryEid = `Category_${childTypeInfo.en}_${id}`
