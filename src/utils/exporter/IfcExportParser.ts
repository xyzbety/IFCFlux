...
import { IfcAPI, Vector, Color } from 'web-ifc'
...
import { IfcElementTypeIds, IfcGeometryTypeIds, IfcTypes, getIfcTypeName } from '../common/ifcTypes';
...
export class IfcExportParser {
  ...
  constructor(props: IFCParserProps) {
    ...
    this.ifcapi.SetWasmPath(wasmDirectory, true) // 设置 wasm 文件的路径
  ...
  public async getAllElements(projectGuid: any) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (DummyElementsSet.includes(value.type)) {
        ...
        entityId++;
      } else if (IfcTypes[this.invertTypeMap[value.type]]?.isElement) {
  ...
  public invertMap(): void {
    ...
    Object.keys(IfcTypes).forEach((key) => {
      const typeId = Number(key);
      const typeName = getIfcTypeName(typeId);
      if (typeName) {
        invertedMap[typeName] = typeId;
      }
    });
  ...
  async getAllTypesOfModel() {
    ...
    const elements = IfcElementTypeIds
  ...
  async getItemProperty(id: number) {
    try {
      ...
      if (props.type) {
        ...
        props.type = Number.isFinite(typeId) ? getIfcTypeName(typeId) : props.type
      }
  ...
  async getAllGeometriesIds() {
    ...
    const geomTypesArray = IfcGeometryTypeIds; // 从统一类型表中获取几何类型集合
